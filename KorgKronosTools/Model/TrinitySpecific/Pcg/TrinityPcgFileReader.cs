...
// ReSharper disable once UnusedParameter.Local; check is used but only for a debug check.
             bool check = true)
        {
            ...
            int size = Util.GetInt(CurrentPcgMemory.Content, _index, 4);
            if (banks == 0)
            {
                patchesPerBank = 0;
            }
            else
            {
                ...
                double patchesPerBankFloat = banks == 0 ? 0.0 : (double) size / patchSize / banks;
        ...
        private void ReadSProgramBanks(int banks, int patches)
        {
            ...
            for (int bankIndex = 0; bankIndex < banks; bankIndex++)
            {
                Debug.Assert(Util.GetInt(CurrentPcgMemory.Content, _index, 2) == 4);
                _index += 2;

                IProgramBank bank = (IProgramBank) CurrentPcgMemory.ProgramBanks[4]; // Fixed
                //bank.PcgId = bankId;
                bank.ByteOffset = _index;
