...
namespace PcgTools.Model.KronosSpecific.Song
{
    ...
    public class KronosSongMemory : SongMemory
    {
        ...
        public KronosSongMemory(string fileName)
            : base(fileName)
        {
            Model = Models.Find(Models.EOsVersion.Kronos2x); // Songs are always considered Kronos 2.x files
        }
        ...
        public override string ProgramIdByIndex(int rawBankIndex, int rawProgramIndex)
        {
            ...
            string bankId = (rawBankIndex >= 0) && (rawBankIndex < names.Length) ? names[rawBankIndex] : Strings.Unknown;
            int indexId = (rawProgramIndex >= 6 && rawProgramIndex <= 16) ? rawProgramIndex + 1 : rawProgramIndex;
